...
export default function InputModalTabContent(props: Partial<ModalProps>){
  const [currencies, setCurrencies] = createSignal(["EUR", "GBP", "USD", "CAD", "RON"]);
    ...
    <BaseCard>
      <FormGroup>
        <InputGroup class="mb-3">
          <DropdownButton title="Currency" variant="outline-secondary" id="button-addon1">
            <Index each={currencies()}>
              {(item) =>
                <DropdownItem>{item as any}</DropdownItem>
              }
            </Index>
          </DropdownButton>
